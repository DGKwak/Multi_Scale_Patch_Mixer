...
# Convert Every MLP Block into one Linear layer and activation
# Weighted sum -> Sum
...
class ShiftBlock(nn.Module):
    def __init__(self,
        ...
        self.channel_mixer_S = MlpBlock(patch_dim, patch_dim*2, self.act, self.dropout)
    ...
    def forward(self, x):
        ...
        z = self.channel_mixer_S(x)

        x_l = channel_shift(z[:, :, :C], shift=self.shift_l, shift_size=self.shift_size)
        x_r = channel_shift(z[:, :, C:C*2], shift=self.shift_r, shift_size=self.shift_size)

        x_l = self.channel_mixer_l(x_l)
        x_r = self.channel_mixer_r(x_r)

        x_all = torch.stack([x_l, x_r], dim=-1)  # (B, N, C, 2)
        z = torch.sum(x_all, dim=-1)
...
class MultiscaleMixer(nn.Module):
    ...
    def __init__(self,
        ...
        self.token_mixer = nn.ModuleList([
            nn.ModuleList([
                nn.Sequential(
                    MlpBlock(x, x*2, self.act, self.dropout),
                    nn.Linear(x*2, x),
                )
