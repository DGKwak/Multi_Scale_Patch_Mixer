...
class ShiftAttention(nn.Module):
    def __init__(self,
        ...
        self.mlp1 = nn.Linear(patch_dim, patch_dim, bias=False)
        self.activation = get_activation(act)
        self.mlp2 = nn.Linear(patch_dim, patch_dim * k, bias=False)
        self.softmax = nn.Softmax(1)

    def forward(self, x):
        B, K, C, N = x.shape

        x = einops.rearrange(x, 'b k c n -> b k n c')
        
        att = torch.sum(torch.sum(x, 1), 1)                         #b, c
        att = self.mlp2(self.activation(self.mlp1(att)))            #b, kc
        att = einops.rearrange(att, 'b (k c) -> b k c', k=self.k)   #b, k, c
        att = self.softmax(att)                                     #b, k, c
        attention = att.unsqueeze(-2)                               #b, k, 1, c
        
        out = attention * x                                         #b, k, n, c
        out = torch.sum(out, 1)                                     #b, n, c
...
class ShiftBlock(nn.Module):
    def __init__(self,
        ...
        self.shift_attention = ShiftAttention(patch_dim, k=3, act=self.act)
    ...
    def forward(self, x):
        ...
        # B, 3, C, N
        z = torch.stack([x_l, x_r, x], dim=1)
        z = self.shift_attention(z)
