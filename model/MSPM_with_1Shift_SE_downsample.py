...
class MultiscaleMixer(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_dim = [patch_dim*2**i for i in range(len(self.num_layers))]
        ...
        self.blocks = nn.ModuleList([
            nn.ModuleList([
                BasicLayer(patch_dim=p,
                           ...
                           act=self.act)
                for idx, (l, p) in enumerate(zip(self.num_layers, self.patch_dim))
            ])
            for x in self.num_patches
        ])
        ...
        self.head = nn.Sequential(
            nn.LayerNorm(self.patch_dim[-1]),
            nn.Linear(self.patch_dim[-1], self.patch_dim[-1]//2),
            nn.Linear(self.patch_dim[-1]//2, 6)
        )
