...
def make_datasets(train_dir,
    ...
    # 데이터셋 로드
    train_dataset = Sobel_dataset(data_path=train_dir, is_train=True)
    val_dataset = Sobel_dataset(data_path=val_dir, is_train=False)
    test_dataset = Sobel_dataset(data_path=test_dir, is_train=False)
...
@hydra.main(config_path='./config', config_name='CSM_config', version_base=None)
def main(cfg):
    metadata = {
    ...
    logger.info(f"클래스 수: {len(train_dataset.get_classes())}")
    logger.info(f"클래스 목록: {train_dataset.get_classes()}")
    ...
    if os.path.exists(best_model_path):
        ...
        cm_path = plot_confusion_matrix(targets, predictions, train_dataset.get_classes(), 
        ...
        with open(test_results_path, 'w') as f:
            ...
            f.write(classification_report(targets, predictions, target_names=train_dataset.get_classes()))
