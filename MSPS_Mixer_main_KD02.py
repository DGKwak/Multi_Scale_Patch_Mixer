...
import model.MSPS_Mixer_KD_teacher as MSPS
import model.MSPS_Mixer_KD_student as MSPS_rev01
...
def train(student_model,
    ...
    for (x, y), (x_t, y_t) in zip(student_loader, teacher_loader):
        ...
        optimizer.zero_grad()
        logit, st_ds = student_model(x)
        t_logit, t_ds = teacher_model(x_t)
...
@hydra.main(config_path='./config', config_name='MSPS_Mixer_KD02', version_base=None)
def main(cfg):
    metadata = {
