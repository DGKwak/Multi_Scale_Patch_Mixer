...
def extract_sobel_features(data):
    ...
    result = np.stack([Dx, Dy, Dxy], axis=0)
...
if __name__ == "__main__":
    ...
    for cat in category:
        ...
        for img_path, label in tqdm(data_sample, desc=f"Processing {cat}"):
            ...
            sobel_img = extract_sobel_features(img)
            dwt_1, dwt_2, dwt_3 = apply_dwt_augmentation(img, wavelet='haar')

            sobel_dwt_1 = extract_sobel_features(dwt_1)
            sobel_dwt_2 = extract_sobel_features(dwt_2)
            sobel_dwt_3 = extract_sobel_features(dwt_3)
            ...
            np.save(save_path_sobel, sobel_img)
            np.save(save_path_dwt_1, sobel_dwt_1)
            np.save(save_path_dwt_2, sobel_dwt_2)
            np.save(save_path_dwt_3, sobel_dwt_3)